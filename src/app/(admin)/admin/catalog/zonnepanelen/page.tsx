...
export default function AdminZonnepanelenPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/catalog/zonnepanelen?page=1&limit=10")
      .then((res) => res.json())
      .then(setInitialData)
      .catch((error) => {
    ...
    <CatalogTable<Zonnepaneel>
      ...
      renderHeader={() => (
        <>
          ...
          <th className="text-left p-3 font-medium">Warranty (years)</th>
      ...
      renderRow={(item, onEdit, onDelete) => (
        <>
          ...
          <td className="p-3">
            {item["Lengte (mm)"]}x{item["Breedte (mm)"]}x{item["Hoogte (mm)"]}
          </td>
          ...
          <td className="p-3 font-mono text-sm">{item["Product code"]}</td>
          <td className="p-3">
