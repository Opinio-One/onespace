...
export default function AdminThuisbatterijenPage() {
  const [initialData, setInitialData] =
    useState<PaginatedResponse<Thuisbatterij> | null>(null);

  useEffect(() => {
    // Fetch initial data from API route
    fetch("/api/catalog/thuisbatterijen?page=1&limit=10")
      .then((res) => res.json())
      .then(setInitialData)
      .catch((error) => {
    ...
    <CatalogTable<Thuisbatterij>
      ...
      renderRow={(item, onEdit, onDelete) => (
        <>
          ...
          <td className="p-3">{item["Batterij Voltage (Vdc/Vac)"]}</td>
